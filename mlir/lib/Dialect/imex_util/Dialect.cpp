...
struct CmpIOfSelect : public mlir::OpRewritePattern<mlir::arith::CmpIOp> {
  ...
  matchAndRewrite(mlir::arith::CmpIOp op,
                  mlir::PatternRewriter &rewriter) const override {
    if (op.getPredicate() != mlir::arith::CmpIPredicate::ne)
      return mlir::failure();

    auto select = op.getLhs().getDefiningOp<mlir::arith::SelectOp>();
    if (!select)
      return mlir::failure();

    if (select.getFalseValue() != op.getRhs())
      return mlir::failure();
...
struct SelectOfSelect : public mlir::OpRewritePattern<mlir::arith::SelectOp> {
  using OpRewritePattern::OpRewritePattern;

  mlir::LogicalResult
  matchAndRewrite(mlir::arith::SelectOp op,
                  mlir::PatternRewriter &rewriter) const override {
    auto prev = op.getTrueValue().getDefiningOp<mlir::arith::SelectOp>();
    if (!prev)
      return mlir::failure();

    if (op.getCondition() != prev.getCondition() ||
        op.getFalseValue() != prev.getFalseValue())
      return mlir::failure();
